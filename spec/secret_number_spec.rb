...
RSpec.describe "secret_number.rb" do
  describe "output" do
    it "asks for a guess at least once", points: 1 do
      output = run_script_and_capture_lines("secret_number.rb", stdin: ["5", "7"].join("\n"))
      expect(output[0]).to eq("Guess the secret number:")
    end

    it "prints 'too low' for a low guess", points: 1 do
      output = run_script_and_capture_lines("secret_number.rb", stdin: ["-1"].join("\n"))
    ...
    it "prints success message when guessed correctly", points: 2 do
      output = run_script_and_capture_lines("secret_number.rb", stdin: (1..10).map(&:to_s).join("\n"))
